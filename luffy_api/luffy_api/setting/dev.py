...
# print(sys.path)
# manage.py 所在目录（与 scripts/build_course_vectors.py 保存 data 的路径一致）
LUFFY_API_ROOT = os.path.dirname(BASE_DIR)


def _load_local_dotenv():
    """本地可选加载 luffy_api/.env（不覆盖已在环境中的变量）。"""
    path = os.path.join(LUFFY_API_ROOT, '.env')
...
# django-cors-headers：沿用库自带默认允许的请求头（含 accept），再追加项目用到的头，
# 避免手写元组遗漏 accept → 浏览器预检失败被误判为「跨域问题」。
from corsheaders.defaults import default_headers as _default_cors_headers

# Django SECRET_KEY：线上务必通过环境变量覆盖
SECRET_KEY = os.environ.get(
    'DJANGO_SECRET_KEY',
    ')xdu2e@a(^1p5ohypkgtft19v*5slt(-m_$gd_o637%^a3f^m(',
)

# 智谱 GLM / Embedding API Key（勿写入仓库）
...
# 开发环境允许任意 Host（ngrok 域名每次重启都会变，不能写死）
ALLOWED_HOSTS = ['*']
...
MIDDLEWARE = [
    ...
    # 必须尽量靠前（在 CommonMiddleware 之前），否则会话/预检请求的 CORS 头容易缺失 → 前端联调报跨域
...
# 允许的请求头（在库默认基础上追加；不要漏掉 axios 常会带的 accept）
...
# simplejwt 配置（替代旧的 djangorestframework-jwt）
from datetime import timedelta
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME': timedelta(minutes=30),   # access token 有效期
    'REFRESH_TOKEN_LIFETIME': timedelta(days=14),    # refresh token 有效期
    'ROTATE_REFRESH_TOKENS': True,                   # 刷新时发新的 refresh token
    'BLACKLIST_AFTER_ROTATION': True,                # 旧 refresh token 刷新后加入黑名单
    'AUTH_HEADER_TYPES': ('JWT', 'Bearer'),          # 同时支持 JWT 和 Bearer 前缀
...
SPECTACULAR_SETTINGS = {
    ...
    # JWT 认证：Swagger UI 中可填入 token 进行接口调试
    'SECURITY': [{'jwtAuth': []}],
    ...
    'TAGS': [
        ...
        {'name': '购物车', 'description': '添加、列表、移除、清空、结算'},
